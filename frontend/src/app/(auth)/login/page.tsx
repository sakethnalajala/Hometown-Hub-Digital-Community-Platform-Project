...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // DEMO MODE BYPASS - Call backend login to get valid JWTs
      ...
      // 2. Exchange token with our backend to get JWT and user profile
        ...
        <div className="space-y-3 relative z-10">
          ...
          {process.env.NEXT_PUBLIC_DEMO_MODE === 'true' && (
            <Button 
              ...
              variant="outline"
              onClick={() => {
                setFormData({ email: 'demo@hometownhub.com', password: 'Demo@12345' })
                setTimeout(() => {
                  const form = document.querySelector('form')
                  if(form) form.dispatchEvent(new Event('submit', { cancelable: true, bubbles: true }))
                }, 100)
              }}
              className="w-full bg-purple-500/10 hover:bg-purple-500/20 border-purple-500/30 text-purple-200 rounded-xl py-6 transition-all"
            >
